<?php
	...
	foreach($json["applist"]["apps"]["app"] as $applist) {	
            ...
            if(stripos($applist["name"], $name) !== false) {
                    $entryname = $applist["name"];
                    $entryid = $applist["appid"];

                    $steam = getFromSteamByID($entryid);	//Get Screenshots, Infotext etc.

                    if($steam != "false"){
                            $imagepath = "img/steam/".$entryid.".jpg";
                            ...
                            VALUES (?, ?, ?, ?)");
                            $result = $statement->execute([$entryid, $imagepath, $entryname, $steam["description"]]);
